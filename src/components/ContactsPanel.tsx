...
import { Plus, PlusCircleIcon, SendHorizonal, X, XCircleIcon } from 'lucide-react';
...
const ContactsPanel = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${BACKEND_URL}/contacts`, {
      ...
      .then((res) => res.json())
      .then((data) => {
        setUsers(data); // adjust if your API returns { users: [...] }
      })
      .catch((err) => {
  ...
  return (
    <div className="p-4 0 text-white bg-[#1a1a1e]">
        <div className={`overflow-y-scroll ${selectedUser && uploadedFiles.length==0?'h-[calc(100vh-280px)]':uploadedFiles.length>=1?`h-[calc(100vh-${infoDivHeight+150}px)]`:'h-[calc(100vh-150px)]'}`}
...
>
      {users.length>0 && users.map((user, index) => (
        ...
        >
            ...
            /> */}
            <ChatAvatar name={user.name} avatar={`${BACKEND_URL}/chat_photo/${user.chat_id}`} backupAvatar={`${BACKEND_URL}/contact_photo/${user.id}`}/>
            <img
          ...
          <span className="hover:text-[#ffffff] text-[#ffffff60]">
            {user.chatTitle?user.chat_title :(user.first_name || user.last_name)?`${user.first_name?user.first_name:''} ${user.last_name?user.last_name:''}`:user.username?user.username : "No name"}
...
{selectedUser && <div ref={infoDivRef} className="p-4 text-white rounded-[8px]">
  {uploadedFiles.length > 0 && (
      ...
      <div className="flex justify-start gap-2 items-center bg-[#ffffff04] rounded-t-[8px] p-2">
        <h2 className="text-sm text-[#fafafa]">Chat with {selectedUser.chatTitle?selectedUser.chat_title :(selectedUser.first_name || selectedUser.last_name)?`${selectedUser.first_name?selectedUser.first_name:''} ${selectedUser.last_name?selectedUser.last_name:''}`:selectedUser.username?selectedUser.username : "No name"}</h2>
      ...
      {showAttachMenu && (
        ...
        <PlusCircleIcon className="w-5 h-5 mr-2 cursor-pointer" onClick={() => setShowAttachMenu((prev) => !prev)}/>
        <input
  type="text"
  placeholder={`Send a message to ${selectedUser.chatTitle?selectedUser.chat_title :(selectedUser.first_name || selectedUser.last_name)?`${selectedUser.first_name?selectedUser.first_name:''} ${selectedUser.last_name?selectedUser.last_name:''}`:selectedUser.username?selectedUser.username : "No name"} on ${selectedUser.platform || "Telegram"}`}
