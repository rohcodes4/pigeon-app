...
const handleSave = async () => {
  const filterNameInput = document.querySelector('input[placeholder="Filter Name"]') as HTMLInputElement;
  const keywordInput = document.querySelector('input[placeholder="Keyword"]') as HTMLInputElement;
...
const updateSmartFilter = async (filterId: string, filterData: {
  name?: string;
  channels?: number[];
  keywords?: string[];
}) => {
  ...
  if (filterData.channels) formData.append('channels', filterData.channels.join(','));
  if (filterData.keywords) formData.append('keywords', filterData.keywords.join('\n'));
...
const handleEditSave = async () => {
  const filterNameInput = document.querySelector('input[placeholder="Edit Filter Name"]') as HTMLInputElement;
  const keywordInput = document.querySelector('input[placeholder="Edit Keywords"]') as HTMLInputElement;
  ...
  try {
    const updatedFilter = await updateSmartFilter(editingFilter.id, {
      ...
      keywords: keywordInput?.value ? keywordInput.value.split(',').map(k => k.trim()) : editingFilter.keywords
    });
    
    // Update the filter in the list
    setSmartFilters(prev => prev.map(f => f.id === editingFilter.id ? updatedFilter : f));
...
<div className="px-2 mb-4">
  ...
  <div className="mt-2">
    {isLoadingFilters ? (
      <div className="text-[#ffffff80] text-sm p-2">Loading filters...</div>
    ) : filterError ? (
      <div className="text-red-400 text-sm p-2">Error: {filterError}</div>
    ) : smartFilters.length === 0 ? (
      <div className="text-[#ffffff80] text-sm p-2">No filtered streams yet</div>
    ) : (
...
>
  <option value="contains" style={{ backgroundColor: '#2d2d2d', color: 'white' }}>Contains</option>
  <option value="not_contains" style={{ backgroundColor: '#2d2d2d', color: 'white' }}>Does Not Contain</option>
  <option value="equals" style={{ backgroundColor: '#2d2d2d', color: 'white' }}>Equals</option>
  <option value="not_equals" style={{ backgroundColor: '#2d2d2d', color: 'white' }}>Does Not Equal</option>
  <option value="starts_with" style={{ backgroundColor: '#2d2d2d', color: 'white' }}>Starts With</option>
  <option value="ends_with" style={{ backgroundColor: '#2d2d2d', color: 'white' }}>Ends With</option>
</select>
  ...
  <div className="mt-2">
    {isLoadingFilters ? (
      <div className="text-[#ffffff80] text-sm px-4 py-2">Loading filters...</div>
    ) : filterError ? (
      <div className="text-red-400 text-sm px-4 py-2">Error: {filterError}</div>
    ) : smartFilters.length === 0 ? (
      <div className="text-[#ffffff80] text-sm px-4 py-2">No filtered streams yet</div>
    ) : (
  ...
  <div key={filter.id} className="relative group">
    ...
    >
      ...
      <div className="flex-1 text-left">
        <div className="flex justify-between items-center">
          <span className="text-[#ffffff48] font-200">{filter.name}</span>
        </div>
        <div className="flex justify-between items-center">
          <span className="text-sm text-[#ffffff32] font-100">
      ...
      <div className="flex flex-col items-center gap-1">
        <span className="self-end text-xs text-gray-400">now</span>
      </div>
    </button>
